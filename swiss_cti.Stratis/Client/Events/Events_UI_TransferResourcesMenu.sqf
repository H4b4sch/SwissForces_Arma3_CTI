...
switch (_action) do {
	case "onLoad": {
		...
		if (missionNamespace getVariable "CTI_AI_COMMANDER_ENABLED" == 1 && !isplayer leader _commander && !isNull _commander) then {
			_AI_commander = _logic getVariable "cti_commander";
			_groups = [_AI_commander] + _groups;
		};
