...

{
	...
	if(missionNamespace getVariable "CTI_UNITS_VAS" == 1) then {
		_hq addAction   ["<t color='#ff1111'>Virtual Ammobox</t>", "VAS\open.sqf"];  //--- Virtual Ammobox in HQ avaiable
	};
...
[] Spawn {
	...
	if (_hour == 24) then {_hour = floor (random 23)};
	if (_month == 13) then {_month = floor (random 12)};
	while {_month == 0} do {_month = floor (random 12)}; //--- Month has range 1-12 whereas random 12 has range of 0-12. however im probably having a special moment and theres probably a simpler way of doing this. Sari.
