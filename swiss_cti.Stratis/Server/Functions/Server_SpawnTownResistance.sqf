...
_pool = [];
{
	...
	if !(isNil {missionNamespace getVariable _unit}) then {
		for '_i' from 1 to _presence do { [_pool, [missionNamespace getVariable _unit, if (count _x > 2) then {_x select 2} else {100}] ] call CTI_CO_FNC_ArrayPush };
	};
} forEach _pool_units;
