...
with missionNamespace do {
	...
	if (isNil 'CTI_AI_COMMANDER_ENABLED') then {CTI_AI_COMMANDER_ENABLED = 0}; //--- Determine whether AI Commander is enabled or not
};
//-----------------------------------------------------------------------------------------------------------------------//
...
//--- Gear: Parameters
CTI_GEAR_RESELL_TAX = 0.3; //--- Owned items are traded for: <item price> * <tax>
//-----------------------------------------------------------------------------------------------------------------------//
...
CTI_TOWNS_CAPTURE_BOUNTY_RATIO = 1; //--- A value above 1 will increase the bounty awarded for town capture (Town value * ratio)
CTI_TOWNS_CAPTURE_RANGE = 50; //--- The range which a unit/vehicle has to be from a town center to capture it
CTI_TOWNS_CAPTURE_VALUE_CEIL = 100; //--- The town value's ceiling
//CTI_TOWNS_CAPTURE_VALUE_ITERATE = 5; //--- The iterated value, (try to match CTI_TOWNS_CAPTURE_VALUE_CEIL), proc all 5 seconds. //--- No longer required. Sari.

//--- Towns: Camps
CTI_TOWNS_CAMPS_CAPTURE_BOUNTY = 100; //--- Bounty received by player whenever he capture a camp.
...
CTI_TOWNS_OCCUPATION_DETECTION_RANGE = 550; //--- Determine how far a threat may be detected from the town center
...
CTI_TOWNS_RESISTANCE_DETECTION_RANGE = 550; //--- Determine how far a threat may be detected from the town center
...
//--- Towns: Parameters
with missionNamespace do {
	if (isNil "CTI_TOWNS_AMOUNT") then {CTI_TOWNS_AMOUNT = 4}; //--- Amount of towns (0: Very small, 1: Small, 2: Medium, 3: Large, 4: Full).	
...
CTI_BASE_STRUCTURE_RESELL_RATIO = 0.5; //--- Ratio of building cost to be refunded when sold
...
CTI_VEHICLES_HOOKERS_LIGHT = []; //--- Light Lifters
CTI_VEHICLES_HOOKERS_MEDIUM = []; //--- Medium Lifters
CTI_VEHICLES_HOOKERS_HEAVY = ["B_Heli_Transport_01_F", "O_Heli_Light_02_unarmed_F", "I_Heli_Transport_02_F", "I_Heli_light_03_unarmed_F"]; //--- Heavy Lifters

//--- Types of liftable Vehicles 
CTI_VEHICLES_HOOKABLE_LIGHT = []; //--- Vehicles which may be carried by light lifters
CTI_VEHICLES_HOOKABLE_MEDIUM = []; //--- Vehicles which may be carried by medium lifters
CTI_VEHICLES_HOOKABLE_HEAVY = ["Car","Motorcycle","Tank","Ship"]; //--- Vehicles which may be carried by heavy lifters
...
with missionNamespace do {
	...
	if (isNil "CTI_ENVIRONMENT_STARTING_HOUR") then {CTI_ENVIRONMENT_STARTING_HOUR = 10}; //--- Starting Hour of the day. 24 = Random hour.
	...
	CTI_GAMEPLAY_VOTE_TIME = if (CTI_Debug) then {8} else {20};
